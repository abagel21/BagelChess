...
if __name__ == '__main__':
    ...
    df = pd.read_csv('../chessEngine/util/partially_processed_standard_2020/pgn_split/pgn_split_3.csv')
    df = df[int(len(df)/2):]
myTemp = pd.DataFrame()
...
def expandGame(board):
    ...
    y = board.copy()
    y.pop()
    myTemp = myTemp.append({'board' : y, "FEN" : y.fen()}, ignore_index = True)
    expandGame(y)
def expandWhiteGames(board):
    ...
    y = board.copy()
    y.pop()
    df = df.append({'board' : y, "FEN" : y.fen()}, ignore_index = True)
    expandWhiteGames(y)
...
def evalBoard(board):
    ...
    print(count)
    count += 1
    k = engine.analyse(board, chess.engine.Limit(time=0.1))['score'].relative.__str__()
...
if __name__ == '__main__':
    ...
    df['FEN'] = df['board'].apply(lambda x : x.fen())
    ...
    print("Expanding the games took " + str((end - start)/60.0) + " minutes")
    print("FINISHED PARALLELIZING")
    print(len(df))
    # df['board'] = df['board'].apply(lambda x : reverseTurn(x))
    df.drop("board", axis=1).to_csv("expanded_2020_3_1.csv", index=False)